...
{
public:
  ExperienceMapScene(int exp_map_size, const std::string& media_path, const std::string& image_file, ExperienceMap *in_map) :
      exp_map_scene(NULL), exp_map_path(NULL), exp_map_goal_path(NULL), exp_map_exps(NULL), map(in_map)
  {
    ...
    device->setWindowCaption(L"Experience Map");
