...
{
public:
  PosecellScene(int posecells_size_in, const std::string& media_path_in, const std::string& image_file_in, PosecellNetwork *in_pc) :
  ...
  window_height = 400;
    ...
    device->setWindowCaption(L"Pose Cell Network");

    driver = device->getVideoDriver();
    scene = device->getSceneManager();

    media_path = media_path_in;
